...
def test_server_fixture(capsys, server):
    ...
    submitted_changeinfo = {
        change["change"]: repo.perforce.run_describe(change["change"])[0] for change in submitted_changes
    }
    ...
    assert submitted_changeinfo == {
        "1": {"action": ["add"], "depotFile": ["//depot/file.txt"], "desc": "Initial Commit"},
        "2": {"action": ["add"], "depotFile": ["//stream-depot/main/file.txt"], "desc": "Initial Commit to Stream\n"},
    ...
    shelved_changeinfo = {
        change["change"]: repo.perforce.run_describe("-S", change["change"])[0] for change in shelved_changes
    }
    # Filter info to only contain relevant keys for submitted changes
    shelved_changeinfo = {
        change: {key: info.get(key) for key in ["depotFile", "desc", "action"]}
    ...
    labelinfo = {label.get("label"): {key: label.get(key) for key in ["Revision"]} for label in labels}
...
def test_checkout(server, tmpdir):
    ...
    repo.sync()
    assert sorted(os.listdir(tmpdir)) == sorted(["file.txt", "p4config"]), "Workspace sync not as expected"
...
def test_stream_switching(server, tmpdir):
    ...
    repo.sync()
    assert len(synced) > 0, "Didn't sync any files"
    assert set(os.listdir(tmpdir)) == set(["file.txt", "p4config"])  # file_2.txt was de-synced
    with open(os.path.join(tmpdir, "file.txt")) as content:
...
def test_stream_switching_migration(server, tmpdir):
    ...
    with tempfile.TemporaryDirectory(prefix="bk-p4-test-") as second_client:
        ...
        repo.sync()
        assert len(synced) > 0, "Didn't sync any files"
        assert set(os.listdir(second_client)) == set(["file.txt", "p4config"])  # file_2.txt was de-synced
